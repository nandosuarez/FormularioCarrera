...
function normalizeStoredValue(value) {
  ...
  if (value && typeof value === "object") {
    return Object.fromEntries(
      Object.entries(value).map(([key, entryValue]) => [key, normalizeStoredValue(entryValue)])
    );
  }
...
async function repairStoredText() {
  ...
  db.data = normalizeStoredValue(db.data);
...
export function listRegistrations({ status = "all", query = "" } = {}) {
...
export function findRegistrationByDocumentOrEmail({ documentNumber, email }) {
  const normalizedDocument = documentNumber.trim().toLowerCase();
  const normalizedEmail = email.trim().toLowerCase();

  return db.data.registrations.find(
    (registration) =>
...
export function getRegistrationById(id) {
  return db.data.registrations.find((registration) => registration.id === id);
}
